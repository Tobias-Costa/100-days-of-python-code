...
# Admin only decorator function -> Decorator used in add_new_post,edit_post and delete_post routes
def admin_only(f):
    @wraps(f)
...
def register():
    ...
    if register_form.validate_on_submit():
        hashed_password = generate_password_hash(method="pbkdf2:sha256", password=register_form.password.data, salt_length=8)

        result = db.session.execute(db.select(User).where(User.email == register_form.email.data)).scalar()
        if result != None:
            flash("You've already signed up with that email! Try log in instead.")
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        if user is None:
            flash("There is no account with that email. Try again!")
            return redirect(url_for("login"))
